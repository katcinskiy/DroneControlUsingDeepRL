...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        bases = [self.actors[i]['actor'](state) for i in range(self.n_actions)]
        mu1 = self.mu1(base1)
        mu2 = self.mu2(base2)
        mus = tuple([self.actors[i]['mu'](bases[i]) for i in range(self.n_actions)])
        mu_history.append(mu1)
        var1 = self.var1(base1)
        var2 = self.var2(base2)
        vars = tuple([self.actors[i]['var'](bases[i]) for i in range(self.n_actions)])
        dist = torch.distributions.multivariate_normal.MultivariateNormal(torch.cat((mu1, mu2), 1), torch.diag_embed(torch.cat((var1, var2), 1)))
